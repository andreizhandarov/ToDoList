...
export const Todolist = React.memo((props: PropsType) => {
	const {todolist, tasks, changeFilter, addTask, removeTodolist, changeTodolistTitle, removeTask, changeTaskStatus, changeTaskTitle} = props
	...
	return (
		<div>
			...
			<Box sx={filterButtonsContainerSx}>
				<ButtonMemo variant={todolist.filter === 'all' ? 'contained' : 'text'} color={'inherit'} onClick={()=> changeFilterTasksHandler('all')}>All</ButtonMemo>
				<ButtonMemo variant={todolist.filter === 'active' ? 'contained' : 'text' } color={'primary'} sx={{m: '0 5px'}} onClick={()=> changeFilterTasksHandler('active')} >Active</ButtonMemo>
				<ButtonMemo variant={todolist.filter === 'completed' ? 'contained' : 'text' } color={'secondary'} onClick={()=> changeFilterTasksHandler('completed')}>Completed</ButtonMemo>
			</Box>
		</div>
	)
})
